...
var ajaxifyShopify = (function(module, $) {
  ...
  // Private functions
  var updateCountPrice, flipSetup, revertFlipButton, modalSetup, showModal, sizeModal, hideModal, drawerSetup, showDrawer, hideDrawer, sizeDrawer, loadCartImages, formOverride, itemAddedCallback, itemErrorCallback, cartUpdateCallback, setToggleButtons, flipCartUpdateCallback, buildCart, cartTemplate, adjustCart, adjustCartCallback, createQtySelectors, qtySelectors, scrollTop, toggleCallback, validateQty , refreshCart;
  ...
  updateCountPrice = function (cart) {
    if ($cartCountSelector) {
      ...
      $cartCountSelector.closest('.header-cart-btn').addClass('countvalue');
      $cartCountSelector.closest('.header-cart-btn').addClass('header-cart-btn');


      if (cart.item_count === 0) {

        $cartCountSelector.addClass('hidden-count');
        $cartCountSelector.closest('.header-cart-btn').removeClass('countvalue');
        jQuery('#ajaxifyModal').removeClass('is-visible');
        //$cartCountSelector.closest('.header-cart-btn').removeClass('header-cart-btn');
      }
    }
    if ($cartCostSelector) {
  ...
  cartUpdateCallback = function (cart) {
    ...
    switch (settings.method) {
      case 'flip':
        ...
        break;
      case 'modal':
        buildCart(cart); refreshCart(cart);
        break;
      case 'drawer':
        buildCart(cart);refreshCart(cart);
        if ( !$drawerContainer.hasClass('is-visible') ) {
  ...
  setToggleButtons = function () {
    ...
    if ($toggleCartButton) {
      ...
      $toggleCartButton.on('click', function(e) {
        e.preventDefault();
        var hidden_count = parseInt(jQuery('.cart-count').text());
        //alert(hidden_count);
        switch (settings.method) {
          case 'modal':
            if ( $modalContainer.hasClass('is-visible') ) {
  ...
  refreshCart = function (cart) { // cart dropdown 
    //$(".cart-count").empty();
    var $cartBtn = $(".qty-count");
    var value = $cartBtn.text() || '0';
    var cart_items_html = "";    
    var $cart = $(".cart-dropdown-content");


    //     $cartBtn.text(value.replace(/[0-9]+/,cart.item_count));
    //  console.log('-->>'+$('body').data('money-format'));
    if (cart.item_count == 0) {
      $('.js-empty-cart__message').removeClass('hidden');
      $('.js-cart_content__form').addClass('hidden');

      cart_items_html += '<div class="js-empty-cart__message">'+ 
        '<p class="empty_cart">Your Cart is Empty</p>'+
        '</div>';
    } else {
      console.log('get the value ');
      $('.qty-count').text(cart.item_count);
      $('.js-empty-cart__message').addClass('hidden');
      $('.js-cart_content__form').removeClass('hidden');
      $('.cart__footer').removeClass('hide');
      cart_items_html += '<div class="items-container">';
      $.each(cart.items, function(index, item) {

        var line_id = index + 1;

        cart_items_html +=  '<div class="cart-row" data-line="'+line_id+'"><div class="grid--full">'+
          '<div class="grid-item one-third large--one-quarter">'+
          '<a class="cart-image" href="' + item.url +'">';
        if (item.image) {
          cart_items_html += '<img src="' + item.image.replace(/(\.[^.]*)$/, "_compact$1").replace('http:', '') + '" alt="' + item.title + '" />';             
        }
        cart_items_html += '</a></div>';

        var ttl = item.title.split(" - ");
        console.log(ttl[1]+'======='+ttl[0]);
        cart_items_html += '<div class="grid-item two-thirds large--three-quarters">'+
          '<a href="' + item.url +'">' +ttl[0]+'</a>';
        cart_items_html += '<div class="qty-price">'+item.quantity+' x '+Shopify.formatMoney(item.price, settings.moneyFormat)+'</div></div></div></div>';         

      });
      cart_items_html += '</div>';
      //=============== cart footer =======
      cart_items_html += '<div class="cart__footer"><div class="total-container">'+
        '<div class="">Number of items: <span class="qty-count">'+cart.item_count+'</span></div>'+
        '<div class="">Item total: <span class="cart-subtotal--price">'+
        '<small aria-hidden="true"><span class="money">'+Shopify.formatMoney(cart.total_price, settings.moneyFormat)+'</span></small>'+
        '</span></div></div>';

      cart_items_html += '<div class="checkout-container"><div class="btn-wrap"><a class="btn" href="/checkout">Checkout</a></div><a class="cart-link" href="/cart">View Cart</a></div></div>';

    }

    $('.cart-dropdown-content').html('<div class="cart-inner-content">'+cart_items_html+'</div>');
  };
...
$(document).ready(function() {
  ...
  //======
  var slideControls   = $(this).find('.slide-controls1');
  $('.product-photo-container').slick({ //==== product image slider on product page ===
    slidesToShow: 1,
    slidesToScroll: 1,
    arrows: true,
    dots: false,
    ...
    responsive: [
      {
        ...
        settings: {
          ...
          prevArrow: slideControls.find('.slide-arrow.slick-prev'),
          nextArrow: slideControls.find('.slide-arrow.slick-next'),
  ...
  $('.product-photo-thumbs').slick({  //==== product thumb slider on product page ===
    slidesToShow: 4,
    slidesToScroll: 1,
  ...
  //==============related product slick slider only on mobile 480px on resize ====
  $(window).resize(function(e){
    var slideControls   = $(this).find('.slide-controls01');
    ...
    }else{ //  remove slick slider on resize over 480px  ====
  ...
  if(window.innerWidth <= 767) {
    var slideControls   = $(this).find('.slide-controls01');//====== related product slick slider only on mobile 480px on page load ====
    $('.related-products').slick({
      slidesToShow: 1,
      slidesToScroll: 1,
      appendArrows: slideControls,
    ...
    //==========home page ====
    $(window).resize(function(e){ //  == on resize home page ===
      if (window.innerWidth <= 767) {
        ...
        if($('.home-block-carousel').hasClass('slick-initialized')){
          $('.home-block-carousel').slick('unslick');
        }
      }
    });
    //  == onpage load  home page ===
    if(window.innerWidth <= 767) { 
      var slideControls   = $(this).find('.slide-controls');
      $('.home-block-carousel').slick({
        slidesToShow: 1,
  ...
  //     var form = $('form#addToCartForm-product-template');
  //   	 $.ajax({
  //       url: "/cart/add",
  //       type: "post",
  //       dataType: 'json',                               
  ...
  //      })
  //   })
  $("[data-option-index=0] .swatch-element").click(function(){
    $('.swatch-element').removeClass('unavailable');
    var selected_option_1 = $(this).attr('data-value');
    
    if($("[data-option-index=1] .swatch-element").length > 0 ){
      $("[data-option-index=1] .swatch-element").each(function(){
        var selected_option_2 = $(this).attr('data-value');	
        if($("[data-option-index=2] .swatch-element").length > 0 ){
          var a = false;
          $("[data-option-index=2] .swatch-element").each(function(){
            var selected_option_3 = $(this).attr('data-value');
            var element3 = $(this);
            //console.log(element3);
            
            $('.hidden-variants option').each(function(){
              if($(this).attr('value') == selected_option_1+' / '+selected_option_2+' / '+selected_option_3){
                 console.log($(this).attr('value')+" = "+selected_option_1+' / '+selected_option_2+' / '+selected_option_3);
                a = true;
                console.log("a value "+a);
              }
            })
            if(!a){
              element3.addClass('unavailable');
            }
          })
        }
      })
      if($("[data-option-index=2] .swatch-element.unavailable input:checked").length > 0){
        $("[data-option-index=2] .swatch-element").not('.unavailable').eq(0).find('input').click();
      }
    }
  })
  $("[data-option-index=1] .swatch-element").click(function(){
    $('.swatch-element').removeClass('unavailable');
    var selected_option_1 = $("[data-option-index=0] .swatch-element input:checked").val();
    var selected_option_2 = $(this).attr('data-value');
    if($("[data-option-index=2] .swatch-element").length > 0 ){
      $("[data-option-index=2] .swatch-element").each(function(){
        var selected_option_3 = $(this).attr('data-value');	
        ...
        $('.hidden-variants option').each(function(){
          if($(this).attr('value') == selected_option_1+' / '+selected_option_2+' / '+selected_option_3){
            console.log($(this).attr('value')+'sss')
            a = true
          }
        })
        if(!a){
  ...
  $("[data-option-index=0] .swatch-element input:checked").click();
  $("[data-option-index=1] .swatch-element input:checked").click();
  var result = /[^/]*$/.exec(window.location.href)[0];
  console.log(result)
  if(result == 'challenge#newsletter-footer'){
    setTimeout(function(){
      window.scrollTo(0,0);
    },1000)
  }
  ...
  $(window).bind("load", function() {
    var max = -1;  var max1 = -1; 

    $('.featured-block-wrap-create .grid-item').each(function(){
      var h = $(this).find('.featured-box--title').innerHeight();
      var h1 = $(this).find('p').innerHeight();
      max = h > max ? h : max;
      max1 = h1 > max1 ? h1 : max1;
    });

    $('.featured-block-wrap-create .grid-item').find('.featured-box--title').css('height',max);
    $('.featured-block-wrap-create .grid-item').find('p').css('height',max1);
  ...
  $(".header-logo").hover( // logo
    function () {
      $('body').find('.add-link-class').addClass('link-blur');
      $(this).removeClass('link-blur');
    }, 
    function () {
      $('body').find('.add-link-class').removeClass('link-blur');    
    });


  //==========hover on right link ==========//
  //    $('.add-link-class').hover(
  //   	function () {
  //       $('body').find('.add-link-class').addClass('logo-blur');
  //       $(this).removeClass('logo-blur');
  //   	}, 
  //   	function () {
  //       $('body').find('.add-link-class').removeClass('logo-blur');      
  //      }
  //    );
  ...
  //   $('.add-link-class-sub').hover(
  //   	function () {
  //       $(this).siblings().addClass('logo-blur');
  //       $('body').find('.add-link-class').addClass('logo-blur');
  //       $(this).closest('.add-link-class').removeClass('logo-blur');

  //   	}, 
  //   	function () {
  //       $('.add-link-class-sub').removeClass('logo-blur');
  //       $('body').find('.add-link-class').removeClass('logo-blur');      

  //      }
  //    );
  $('.mobile-nav--has-dropdown,.mobile-nav--has-dropdown .icon-arrow-down').addClass('nav-active')
